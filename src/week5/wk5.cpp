...
void handleEvent(SDL_Event event, DrawingWindow &window) {
	if (event.type == SDL_KEYDOWN) {
		if (event.key.keysym.sym == SDLK_LEFT) { std::cout << "LEFT" << std::endl;  cameraPos.x += 1.0; } 
		else if (event.key.keysym.sym == SDLK_RIGHT) { std::cout << "RIGHT" << std::endl; cameraPos.x -= 1.0; }
		else if (event.key.keysym.sym == SDLK_LSHIFT) { std::cout << "shift" << std::endl; cameraPos.y -= 1.0; }
		else if (event.key.keysym.sym == SDLK_SPACE) { std::cout << "space" << std::endl; cameraPos.y += 1.0; }
		else if (event.key.keysym.sym == SDLK_UP) { std::cout << "UP" << std::endl; cameraPos.z -= 1.0; }
		else if (event.key.keysym.sym == SDLK_DOWN) { std::cout << "DOWN" << std::endl; cameraPos.z += 1.0; }
		...
		else if (event.key.keysym.sym == SDLK_l) {	lookAt(); std::cout << "look at" << std::endl;	}
		else if (event.key.keysym.sym == SDLK_o) { setOrbit(); std::cout << "set orbit" << std::endl;	}
